...
class Template:
    ...
    supporte_composer: bool = False        # ← AJOUTÉ
...
def _mapper_securex_config(fiche: dict, cfg: dict) -> dict:
    ...
    statut = ("Dirigeant" if sit.get("dirigeant") else "Ouvrier" if sit.get("ouvrier") else "Employé")
    ...
    return {
        ...
        "l_table_net": eur_be(net_payer),
        "l_net_employeur": eur_be(net_payer),
        "l_brut_onss_108": eur_be(brut_onss_108),
        "l_table_remu": "", "l_jour_ferie": "", "l_frais_vetements": "",
        "l_rgpt": "", "l_onss_patronale": "",
        "l_table_remu_code": "", "l_table_remu_jours": "", "l_table_remu_heures": "",
        "l_table_remu_label": "", "l_jour_ferie_code": "", "l_jour_ferie_jours": "",
        "l_jour_ferie_heures": "", "l_jour_ferie_label": "", "l_frais_vetements_code": "",
        "l_frais_vetements_nombre": "", "l_frais_vetements_valeur": "",
        "l_frais_vetements_label": "", "l_rgpt_code": "", "l_rgpt_nombre": "",
        "l_rgpt_valeur": "", "l_rgpt_label": "",
        "date_emission_haut": "", "bce_employeur": "", "ref_calcul": "",
        "dernier_traitement": "", "date_en_service": "", "date_anc_firme": "",
        "date_anc_secteur": "", "cp_code": "", "cp_desc": "",
        "qualif_prof_code": "", "qualif_prof_desc": "", "fonction_securex": "",
    }
...
def _mapper_sdworx_config(fiche: dict, cfg: dict) -> dict:
    ...
    valeurs = {
        ...
        "iban": "", "verse_label": "", "bic": "",
        "matricule_haut": "", "matricule_bas": "",
        "pagination": "1/1",
        ...
        "num_travailleur": "", "num_gps": "", "date_entree": "", "date_anciennete": "", "cp": "",
        ...
        "total_verse": eur_be(salaire_net),
        "l_1011": eur_be(brut),
        "l_2500": _signe(-onss),
...
def _mapper_cpas_config(fiche: dict, cfg: dict) -> dict:
    ...
    return {
        ...
        "matricule": "", "entre_le": "", "index": "", "cie_assurances": "",
        "fonction_cpas": "", "paie_du_avec_heure": "", "fonction_code_cpas": "",
        "anciennete_pecuniaire": "", "conjoint_info": "",
        ...
        "cal_cyc_contr_values": "", "cal_cyc_hab_values": "",
        "cal_n10_code": "", "cal_n10_values": "", "cal_preste_flag": "",
        "barcode_data": "", "barcode_central": "", "barcode_bas_gauche": "",
...
def _mapper_sdworx_bbpharma_config(fiche: dict, cfg: dict) -> dict:
    c = fiche["calculs"]
    brut_nominal = _d(cfg.get("brut", "0"))
    imposable = D(c["imposable"])
    precompte = D(c["precompte_net"])
...
def _mapper_tpalm_config(fiche: dict, cfg: dict) -> dict:
    ...
    return {
        ...
        "niss":          cfg["perso"].get("niss",""),
        "periode":       f"{debut} - {fin}",
        "brut":          fmt(brut), "onss": fmt(onss - bon),
        "imposable":     fmt(imp),  "precompte": fmt(prec),
        "net":           fmt(net),  "net_payer": fmt(net),
    }
...
def _mapper_ucm_config(fiche: dict, cfg: dict) -> dict:
    ...
    return {
        ...
        "brut_onss":     eur_be(brut),   "onss_trav":  eur_be(onss - bon),
        "imposable":     eur_be(imp),    "precompte":  eur_be(prec),
        "net_base":      eur_be(net),    "net_payer":  eur_be(net) + " EUR",
    }
...
def _mapper_partena_config(fiche: dict, cfg: dict) -> dict:
    ...
    return {
        ...
        "brut_total":     eur_p(brut),        "onss_trav":      _signe(-onss),
        "red_onss":       eur_p(bon) if bon > 0 else "0,00",
        "cot_trav_total": _signe(-(onss - bon)),
        "imposable":      eur_p(imp),         "precompte":  _signe(-prec),
        "net_final":      eur_p(net),
    }
...
TEMPLATES["hsp_be"] = Template(
    ...
    pdf=RACINE / "hsp.pdf", reperes=_REPERES_HSP, mapper_config=None,
    periode=_periode_hsp, supporte_composer=True,
...
TEMPLATES["bosa_spf"] = Template(
    ...
    pdf=RACINE / "bosa_spf.pdf", reperes=_REPERES_BOSA, mapper_config=None,
    periode=_periode_bosa, supporte_composer=True,
    employeur_fixe={"nom": "SPF Justice", "ville": "1000 Bruxelles"},
    pages_a_garder=[0],
)
TEMPLATES["tpalm_be"] = Template(
    nom="T.PALM — Billet de paie construction",
    pdf=RACINE / "tpalm.pdf", reperes=_REPERES_TPALM, mapper_config=None,
    periode=_periode_tpalm, supporte_composer=True,
    employeur_fixe={"nom": "T.PALM", "ville": "4910 Theux"},
    pages_a_garder=[0],
)
TEMPLATES["ucm_be"] = Template(
    nom="UCM Secrétariat social — Ouvrier transport",
    pdf=RACINE / "ucm.pdf", reperes=_REPERES_UCM, mapper_config=None,
    periode=_periode_ucm, supporte_composer=True,
...
TEMPLATES["partena_be"] = Template(
    ...
    pdf=RACINE / "partena.pdf", reperes=_REPERES_PARTENA, mapper_config=None,
    periode=_periode_partena, supporte_composer=True,
    employeur_fixe={"nom": "CLARAP TRANSPORT", "ville": "4000 Liège"},
    pages_a_garder=[0],
)
TEMPLATES["twinntax_be"] = Template(
    nom="TwinnTax — Dirigeant d'entreprise",
    pdf=RACINE / "twinntax.pdf", reperes=_REPERES_TWINNTAX, mapper_config=None,
    periode=_periode_twinntax, supporte_composer=True, supporte_atn=True,
...
TEMPLATES["liege_be"] = Template(
    ...
    pdf=RACINE / "liege.pdf", reperes=_REPERES_LIEGE, mapper_config=None,
    periode=_periode_liege, supporte_composer=True,
